#ifndef SHARED_ANIMATION_HPP
#define SHARED_ANIMATION_HPP
...
struct Animation {
public:
    Animation(std::initializer_list<std::string> paths);

    std::vector<std::shared_ptr<Util::Image>> imgs;
};

/* 
    !!! similar to PTSD except the frames are shared !!!
*/
class SharedAnimation : public Core::Drawable {
public:
    /**
     ...
     * restart.
     */
    SharedAnimation(const Animation &anime, bool play,
...
class SharedAnimatedGameObject : public GameObjectEx {
public:
    explicit SharedAnimatedGameObject(const SharedRc::Animation& anime);
...
private:
    std::shared_ptr<SharedRc::SharedAnimation> m_Anime;
};
